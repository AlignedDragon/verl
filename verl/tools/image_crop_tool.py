...
class ImageCropTool(BaseTool):
    """A tool for cropping an image based on a bounding box.

    This tool provides a cropping functionality for images,
    with rate limiting and concurrent execution support through Ray.

    Methods:
        get_openai_tool_schema: Return the tool schema in OpenAI format
        create: Create a tool instance for a trajectory
        execute: Execute the cropping operation
    ...
    def __init__(self, config: dict, tool_schema: OpenAIFunctionToolSchema):
        ...
        _tool_schema = OpenAIFunctionToolSchema.model_validate({
            ...
            "function": {
                ...
                "description": (
                    "Crop a specific region of an image based on a bounding box (bbox) and an "
                ...
                "parameters": {
                    ...
                    "properties": {
                        "bbox_2d": {
                            ...
                            "maxItems":4,
                            "description": (
                                "The bounding box of the region to crop, as [x1, y1, x2, y2], where (x1, y1) is "
        ...
        logger.info(f"Initialized ImageCropTool with config: {config}")
    ...
    def _save_image_debug_info(self, image, cropped_image, bbox, resized_bbox, label, output_path):
        ...
        # Save the images
        image_with_box.save(os.path.join(instance_path, "original_with_box.jpg"))
        cropped_image.save(os.path.join(instance_path, "cropped.jpg"))
    ...
    async def create(self, image: str | Image.Image, instance_id: Optional[str] = None, **kwargs) -> str:
        """
        Creates a new instance for image cropping tool.

        This method initializes a new session for an image, which can then be used
        for operations like cropping. It fetches the image from various sources
    ...
    async def execute(self, instance_id: str, parameters: dict[str, Any], **kwargs) -> tuple[dict, float, dict]:
        ...
        try:
            ...
            logger.error(f"Error processing image cropping: {e}")
            return {"text": f"Error processing image cropping: {e}"}, -0.05, {"success": False}

        response_text = f"Cropped the image to the region {bbox_2d}."
        if label:
            response_text = f"Cropped the image to the region {bbox_2d} with label {label}."
