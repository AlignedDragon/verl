...
class ImageLineTool(BaseTool):
                ...
                "parameters": {
                    ...
                    "properties": {
                        "start": {
                            ...
                            "maxItems": 2,
                            "description": (
                                "The starting point of the line, as [x1, y1], where (x1, y1) is the top-left corner."
                            ),
                        },
                        "end": {
                            "type": "array",
                            ...
                            "maxItems": 2,
                            "description": (
                                "The ending point of the line, as [x2, y2], where (x2, y2) is the bottom-right corner."
                            ),
                        }
                    },
                    "required": ["start", "end"],
    ...
    async def execute(self, instance_id: str, parameters: dict[str, Any], **kwargs) -> tuple[dict, float, dict]:
        ...
        try:
            x1, y1, x2, y2 = self._maybe_resize_bbox(start + end, image_width=image_width, image_height=image_height)

            if x1 is None or y1 is None or x2 is None or y2 is None:
                error_msg = (
                    f"Error: The specified coordinates {start} and {end} are invalid "
            ...
            # self._save_image_debug_info(image_with_line, x1, y1, x2, y2, output_path="/users/jsaydali/scratch/run_outputs/image_outputs")
        except Exception as e:
            logger.error(f"Error processing image line-drawing: {e}")
            return {"text": f"Error processing image line-drawing: {e}"}, -0.05, {"success": False}

        response_text = f"Drew a line on the image from {start} to {end}."
