...
class ImageBBoxTool(BaseTool):
    """A tool for drawing bbox on an image based on a bounding box.

    This tool provides a bbox-drawing functionality on an image,
    with rate limiting and concurrent execution support through Ray.

    Methods:
        get_openai_tool_schema: Return the tool schema in OpenAI format
        create: Create a tool instance for a trajectory
        execute: Execute the bbox-drawing operation
    ...
    def __init__(self, config: dict, tool_schema: OpenAIFunctionToolSchema):
        ...
        _tool_schema = OpenAIFunctionToolSchema.model_validate({
            "type": "function",
            "function": {
                "name": "image_bbox_tool",
                "description": (
                    "Draws a bounding box on a specific region of an image based on a bounding box (bbox) and an "
                ...
                "parameters": {
                    ...
                    "properties": {
                        "bbox_2d": {
                            ...
                            "maxItems":4,
                            "description": (
                                "The bounding box of the region to draw, as [x1, y1, x2, y2], where (x1, y1) is "
    ...
    def _save_image_debug_info(self, image, image_with_box, bbox, resized_bbox, label, output_path):
    ...
    async def create(self, image: str | Image.Image, instance_id: Optional[str] = None, **kwargs) -> str:
        ...
        for operations like drawing a bounding box. It fetches the image from various sources
    ...
    async def execute(self, instance_id: str, parameters: dict[str, Any], **kwargs) -> tuple[dict, float, dict]:
        ...
        try:
            ...
            if resized_bbox is None:
                error_msg = (
                    f"Error: The specified bounding box {bbox_2d} is invalid "
